...
namespace Assets.Scripts.Effects
{
    ...
    public class ColorRotator
    {
        ...
        public Color Color { get { return _colorHsv.ToColor(); } }

        public ColorRotator()
        {
            _randomTimeOffset = Random.value * 20;
            _colorHsv = new ColorHSV { A = 100.0f, H = Random.value * 360.0f, S = Random.value * 100.0f, V = Random.value * 100.0f };
        }

        public void Rotate()
        {
            _colorHsv.H += Mathf.PerlinNoise(Time.time + _randomTimeOffset, 0.0f) * 0.25f;
            _colorHsv.S += Mathf.PerlinNoise(Time.time + _randomTimeOffset, 1.0f) * 0.1f;
            _colorHsv.V += Mathf.PerlinNoise(Time.time + _randomTimeOffset, 2.0f) * 0.1f;
        }
    }
}
