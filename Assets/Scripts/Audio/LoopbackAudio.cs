...
namespace Assets.Scripts.Audio
{
    public class LoopbackAudio : MonoBehaviour
    {
        ...
        public float[] SmoothedSpectrumData;
        ...
        public void Awake()
        {
            ...
            {
                _spectrumHistory.Add(new float[AudioSmoothingIterations]);
            }
        ...
        public void Update()
        {
            for (int i = 0; i < SpectrumSize; i++)
            {
                int historyIndex = Time.frameCount % AudioSmoothingIterations;

                float audioData = GetSpectrumData(AudioVisualizationStrategy.PostScaled, i);
                _spectrumHistory[i][historyIndex] = audioData;

                SmoothedSpectrumData[i] = _spectrumHistory[i].Average();
            }
        }
        ...
        public float GetSpectrumData(AudioVisualizationStrategy strategy, int index = 0)
        {
            ...
            switch (strategy)
            {
                ...
                case AudioVisualizationStrategy.PostScaled:
                    ...
                    break;
                case AudioVisualizationStrategy.Smoothed:
                    spectrumData = SmoothedSpectrumData[index];
                    break;
                case AudioVisualizationStrategy.PostScaledMinMax:
